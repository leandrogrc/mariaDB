...
async function getAllLinks() {
  ...
  try {
    ...
    if (result.length == 0)
      return { success: true, response: "Zero links on database!" };
...
async function getLink(linkId) {
  ...
  try {
    ...
    const link = await connection.query("SELECT * FROM links WHERE id = ?", [
      linkId,
    ]);
    console.log(link);
    if (link.length === 0)
...
async function addLinkToUser(userId, link, title) {
  ...
  try {
    ...
    const result = await connection.query(
      "INSERT INTO links (user_id, link, title) VALUES (?, ?, ?)",
      [userId, link, title]
    );
